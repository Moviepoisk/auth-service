...
class Settings(BaseSettings):
    ...
    super_admin_role_description: str = Field(
        "Role with all permissions.", env="SUPER_ADMIN_ROLE_DESCRIPTION"
    )

    admin_role_name: str = Field("admin", env="ADMIN_ROLE_NAME")
    admin_role_description: str = Field(
        "Role with access to admin level functionalities.", env="ADMIN_ROLE_DESCRIPTION"
    )

    user_role_name: str = Field("user", env="USER_ROLE_NAME")
    user_role_description: str = Field(
        "Standard user role with basic access rights.", env="USER_ROLE_DESCRIPTION"
    )

    subscriber_role_name: str = Field("subscriber", env="SUBSCRIBER_ROLE_NAME")
    subscriber_role_description: str = Field(
        "Role for users with subscription.", env="SUBSCRIBER_ROLE_DESCRIPTION"
    )

    guest_role_name: str = Field("guest", env="GUEST_ROLE_NAME")
    guest_role_description: str = Field(
        "Role for unregistered users with minimal access.", env="GUEST_ROLE_DESCRIPTION"
    )

    # sequrity settings jwt
    jwt_access_token_expire_minutes: int = Field(
        60, env="AUTH_ACCESS_TOKEN_EXPIRE_MINUTES"
    )
