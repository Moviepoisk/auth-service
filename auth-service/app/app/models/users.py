...
from sqlalchemy import (
    Column, DateTime, String, ForeignKey, Index, LargeBinary, Text, Boolean
)
...
class RefreshTokenDbModel(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey('users.id'), nullable=False)
    expires_at = Column(DateTime, nullable=False)  # Время истечения срока действия токена
...
RoleDbModel.users = relationship("UsersDbModel", order_by=UsersDbModel.id, back_populates="role")
LoginHistoryDbModel.user = relationship("UsersDbModel", back_populates="login_histories")
UsersDbModel.refresh_tokens = relationship("RefreshTokenDbModel", order_by=RefreshTokenDbModel.created_at, back_populates="user")
UsersDbModel.encryption_keys = relationship("EncryptionKeysModel", order_by=EncryptionKeysModel.created_at, back_populates="user")
