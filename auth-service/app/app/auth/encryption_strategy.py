...
class SessionKeyEncryptor(ABC):
    @abstractmethod
    async def encrypt_session_key(self, session_key: bytes, public_key: RSA.RsaKey) -> bytes:
        pass

    @abstractmethod
    async def decrypt_session_key(self, encrypted_session_key: bytes, private_key: RSA.RsaKey) -> bytes:
        pass
...
class RSAEncryptor(SessionKeyEncryptor):
    async def encrypt_session_key(self, session_key: bytes, public_key: RSA.RsaKey) -> bytes:
        loop = asyncio.get_running_loop()
        enc_session_key = await loop.run_in_executor(None, PKCS1_OAEP.new(public_key).encrypt, session_key)
        return enc_session_key

    async def decrypt_session_key(self, encrypted_session_key: bytes, private_key: RSA.RsaKey) -> bytes:
        loop = asyncio.get_running_loop()
        session_key = await loop.run_in_executor(None, PKCS1_OAEP.new(private_key).decrypt, encrypted_session_key)
        return session_key

class AESEncryptor(DataEncryptor):
    async def encrypt(self, data: str, session_key: bytes) -> EncryptedMessage:
        loop = asyncio.get_running_loop()
        encrypted_message = await loop.run_in_executor(None, lambda: self._encrypt_data(data, session_key))
    ...
    async def decrypt(self, encrypted: EncryptedMessage, session_key: bytes) -> str:
        loop = asyncio.get_running_loop()
        decrypted_data = await loop.run_in_executor(None, lambda: self._decrypt_data(encrypted, session_key))
