...
class AbstractRefreshTokenRepository(ABC):
    ...
    @abstractmethod
    async def get_refresh_token_by_user_id(self, user_id: str) -> Optional[RefreshTokenDbModel]:
...
class RefreshTokenRepository(AbstractRefreshTokenRepository):
    ...
    async def create_refresh_token(self, user_id: str, token: str, expires_at: datetime) -> None:
        refresh_token = RefreshTokenDbModel(user_id=user_id, token=token, expires_at=expires_at, revoked=False)
    ...
    async def get_refresh_token_by_user_id(self, user_id: str) -> Optional[RefreshTokenDbModel]:
        query = select(RefreshTokenDbModel).where(
            ...
            RefreshTokenDbModel.revoked == False
        )
    ...
    async def revoke_refresh_token(self, id: str) -> None:
        await self.db.execute(update(RefreshTokenDbModel).where(
            RefreshTokenDbModel.id == id).values(revoked=True))
        await self.db.commit()
        

    async def delete_refresh_token(self, id: str) -> None:
        await self.db.execute(delete(RefreshTokenDbModel).where(
            RefreshTokenDbModel.token == id))
        await self.db.commit()
