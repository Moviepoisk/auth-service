...
async def register_new_user(db: AsyncSession, user_data: UserCreate) -> str:
    ...
    encrypted_password_message = await data_encryptor.encrypt(user_data.password, session_key)
...
async def authenticate_user(db: AsyncSession, email_or_login: str, password: str) -> Optional[UserGet]:
    ...
    decrypted_password = await aes_encryptor.decrypt(encrypted_message, decrypted_session_key)
...
async def refresh_user_tokens(refresh_token: str, db: AsyncSession):
    ...
    refresh_token_updated = await create_access_and_refresh_tokens(db, refresh_token_verefied.login)
...
async def revoke_refresh_token(db: AsyncSession, acesss_token: str):
    ...
    if refresh_token_db:
        await tokens_repo.revoke_refresh_token(
            refresh_token_db.id
        )
